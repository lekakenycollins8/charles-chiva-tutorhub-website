...
import { featuredResources, Resource } from '@/data/resources';

const FeaturedResources = () => {
  // We already have the featured resources imported from data file
  // Using the first 3 resources if there are more than 3
  const displayResources = featuredResources.slice(0, 3);
    ...
    <section className="py-24 bg-gradient-to-tr from-blue-50 to-white relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {displayResources.map((resource: Resource, index: number) => (
            <div 
              key={resource.id} 
            ...
            >
              <div className="relative h-52 w-full overflow-hidden">
                ...
                <div className={`absolute top-0 right-0 ${resource.isFree ? 'bg-green-500' : 'bg-blue-600'} text-white font-bold px-4 py-2 m-3 rounded-full shadow-md transform -rotate-2`}>
                  {resource.isFree ? 'Free Access' : `£${resource.price}`}
                </div>
              </div>
                ...
                <div className="flex space-x-3">
                  ...
                  <Link href={`/resources/${resource.id}`} passHref className="flex-1">
                    <Button className="w-full bg-blue-600 hover:bg-blue-700">
                      <span>{resource.isFree ? 'Access Now' : 'Purchase'}</span>
                      <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 ml-2" viewBox="0 0 20 20" fill="currentColor">
